...
def seq2seq_test(args, Dte, path, m, n):
    ...
    y, pred,avg_y = np.array(y), np.array(pred),np.array(avg_y)
    y = (m - n) * y + n
    pred = (m - n) * pred + n
    avg_y = (m - n) * avg_y + n
    print('mape:', get_mape(y, pred))
...
def plot(y, pred,avg_y):
    ...
    x = [i for i in range(1, 300 + 1)]
    x2=[i for i in range(2,300+2,3)]
    # print(len(y))
    plt.plot(x, y[:300], c='green', marker='*', ms=1, alpha=0.75, label='true')
    plt.plot(x, pred[:300], c='red', marker='o', ms=1, alpha=0.75, label='pred')
    plt.plot(x2, avg_y[:100], c='blue', marker='o', ms=1, alpha=0.75, label='avg input')
    plt.grid(axis='y')
